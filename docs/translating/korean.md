...
`bot - 봇` `emoticon - 이모티콘` `emoji - 이모티콘` `icon - 아이콘` `link - 링크` `menu - 메뉴`<br>
`mute - 뮤트` `navigation - 네비게이션` `stream - 스트림` `unmute - 언뮤트` `upload - 업로드`
...
- reply to - **에 답장하다**
  ...
  `There are no messages to reply to. = 할 메시지가 없습니다.`
